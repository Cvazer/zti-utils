...
import com.github.cvazer.rest.RESTMessenger;
...
public class HTTPMessenger implements RESTMessenger{
    ...
    public void createContext(String contextURL, HTTPConsumable consumable) {
        ...
        server.createContext(contextURL, exchange -> {
            byte[] received = new byte[exchange.getRequestBody().available()];
            exchange.getRequestBody().read(received);
            byte[] data = consumable.consume(received, exchange.getRequestURI());
    ...
    public byte[] send(String url, byte[] data){
        try {
            ...
            byte[] answer = new byte[connection.getInputStream().available()];
            connection.getInputStream().read(answer);
            return answer;
        } catch (IOException e) {
            return null;
        }
     ...
     */
    @Override
    public byte[] send(byte[] data, Object[] args) {
        try { @SuppressWarnings("unused") String test = (String)args[0]; }
        catch (Exception e) { throw new IllegalArgumentException("First argument should be a String instance");}
